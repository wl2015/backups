...
public class WaiteSureDaoImp extends BaseDao implements WaiteSureDao{
	...
	/*
	 * 查询用户订单列表
	 * @see com.pk.wechat.waitesure.dao.WaiteSureDao#getUserOrderForm(int)
	 */
	
	//and o.send_time between '"+sendStartDa+"'and '"+sendEndDa+"'
	int pageSize=3;//（*自己设置，跟action里的一致）
	public List<OrderForm> getUserOrderForm(int pageNum) {
		//SELECT o.orderform_id,u.name,u.link_phone,o.order_time,o.isdeal FROM orderform o,userinfo u WHERE o.isdeal='0'and o.user_id = u.userinfo_id and "+"order_time between '"+startDate+"' and '"+endDate+"' LIMIT "+pageSize*(pageNum-1)+","+pageSize
		String sql = "SELECT o.orderform_id,u.name,u.link_phone,o.order_time,o.send_time,o.isdeal FROM orderform o,userinfo u WHERE o.isdeal=0 and o.user_id = u.userinfo_id and o.send_time BETWEEN '"+sendStartDa+"'and '"+sendEndDa+"'ORDER BY o.order_time DESC LIMIT "+pageSize*(pageNum-1)+","+pageSize;
	...
	/*
	 * 根据订单ID查找并显示订单详情信息
	 * @see com.pk.wechat.waitesure.dao.WaiteSureDao#getOrderInfoById()
	 */
	...
	/*
	 * 根据订单ID删除该条订单
	 * @see com.pk.wechat.waitesure.dao.WaiteSureDao#DeleteOrderform(int)
	 */
	public int DeleteOrderform(int orderid) {
		int rows = super.update("DELETE FROM orderform WHERE orderform_id = ?", 
				new Object[]{
				orderid,
	...
	/*
	 * 点击确认后修改订单表中is_deal的状态值为1
	 * @see com.pk.wechat.waitesure.dao.WaiteSureDao#update(com.pk.wechat.entity.OrderForm)
	 */
	public int update(int orderid) {
		int rows = super.update("UPDATE orderform SET isdeal='1' WHERE orderform_id=?", 
				new Object[]{
				orderid,
