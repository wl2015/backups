...
public class UserController {
	...
	public Map<String, Object> changePass(HttpServletRequest request) throws Exception{
		...
		//获取数据
		String oldPass = CryptogramUtil.decrypt(ValidateUtil.valiStrIsEmpty(request.getParameter("oldPass")),key);		
		String newPass = CryptogramUtil.decrypt(ValidateUtil.valiStrIsEmpty(request.getParameter("newPass")),key);	
		oldPass = SystemUtil.removeAllTagFromHtml(oldPass);
		newPass = SystemUtil.removeAllTagFromHtml(newPass);
		String reNewPass = CryptogramUtil.decrypt(ValidateUtil.valiStrIsEmpty(request.getParameter("reNewPass")),key);
		...
		//原密码一致性验证
		if(!md5oldPass.equals(loginuPass)){
			resultMap.put("code", Constants.AJAX_FAIL_ALERT_CODE);
			resultMap.put("msg", "原密码错误");
			return 	resultMap;
		}
		//passChanged标志是否修改成功 0:修改没成功，1：密码修改成功
	 ...
	 * @return
	 */
	@RequestMapping("/user/completeInfo")
	@ResponseBody
	public Map<String, Object> completeInfo(HttpServletRequest request) throws Exception{
		/*profession
		city/province/phoneNum/name/organization/writerName*/
		return null;
	}
	/**
	 * 请求注册页面
	...
	public Map<String, Object> checkePhone(HttpServletRequest request) throws Exception{
		...
		phoneNum = (String)SystemUtil.removeAllNotCompleteTagFromHtml(phoneNum);
		logger.debug("\n\n\n\n-----------------------phoneNum:"+phoneNum+"\n\n");
	...
	public Map<String, Object> doLogin(HttpServletRequest request) throws Exception{
		...
		Map<String, Object> resultMap = new HashMap<String, Object>();		
		//非整个form表单提交数据
		String phone = CryptogramUtil.decrypt(ValidateUtil.valiStrIsEmpty(request.getParameter("phoneNum")), key);
		phone = SystemUtil.removeAllTagFromHtml(phone);
		String pass  = CryptogramUtil.decrypt(ValidateUtil.valiStrIsEmpty(request.getParameter("password")), key);
	...
	public Map<String, Object> doResetPass(HttpServletRequest request) throws Exception{
		...
		if( phoneNum.equals(phoneCodePhone) && newPass.equals(reNewPass) 
		    && smsCode.equals(phoneodescode)){
