$(function() {
  ...
  $(".vertify-BTN").click(function clickCheckCode() {
    ...
    $buttonNode.val("正在发送...");
    $buttonNode.parent().css("width", "100px");
    phoneNode.attr('readonly', 'readonly');
    $.ajax( {
      url : WEBROOT + '/merchant/getsmsCode.do',
      type : 'post',
      data : {
        p : phoneNum
      },
      ...
      var successCallback = function() {
        ...
        function timeRun() {
          if (waitTime == 0) {
            ...
            $buttonNode.parent().css("width", "89px");
      ...
      $.ajax( {
        ...
        success : function(res) {
          if (res.code == AJAX_SUCCESS_ALERT_CODE) {
            window.location = WEBROOT + '/merchant/toMerchantRegist2.do';                      
            //alert("success");
          } else {  
            //让用户可重新编辑信息
            $('#vertify-input').val("");
