...
<body>
      ...
      <c:forEach var="item" items="${dishList}" varStatus="status">
          ...
          <div class="contentRight">
            <ul class="contentList">
              ...
              <li><strong>原价：</strong><span>${item.originalPrice }</span></li>
              <li><strong>售价：</strong><span>${item.retailPrice }</span></li>
              <li class="two"><strong>口味：</strong><span>${item.dishTaste }</span><strong class="leixing">类型：</strong><span>${item.typeName }</span></li>
              <li><strong>菜品详细介绍：</strong><span class="introInfo">${item.dishDetail }</span></li>  <!--  -->
            </ul>
          </div>
          <div class="operate">
  ...
  <a href="javascript:void(0)" onclick="contentSet('dish/show.do?pageNow=1')">首页</a>
  <c:choose>
    <c:when test="${page.pageNow - 1 > 0}">
      <a href="javascript:void(0);" onclick="contentSet('dish/show.do?pageNow=${page.pageNow - 1}')" >上一页</a>
    </c:when>
    <c:when test="${page.pageNow - 1 <= 0}">
      <a href="javascript:void(0);" onclick="contentSet('dish/show.do?pageNow=1')">上一页</a>
    </c:when>
  </c:choose>
  <font size="2">第 ${page.pageNow}/ ${page.totalPageCount} 页</font>
  <c:choose>
    <c:when test="${page.pageNow + 1 < page.totalPageCount}">
      <a href="javascript:void(0);" onclick="contentSet('dish/show.do?pageNow=${page.pageNow + 1}')">下一页</a>
    </c:when>
    <c:when test="${page.pageNow + 1 >= page.totalPageCount}">
      <a href="javascript:void(0);" onclick="contentSet('dish/show.do?pageNow=${page.totalPageCount}')">下一页</a>
    </c:when>
  </c:choose>
  <a href="javascript:void(0);" onclick="contentSet('dish/show.do?pageNow=${page.totalPageCount}')">尾页</a>
  <br>
  </div>
