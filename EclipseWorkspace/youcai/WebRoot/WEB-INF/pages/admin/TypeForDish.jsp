...
<title>菜品套餐管理----菜品分类</title>
<link rel="stylesheet" type="text/css" href="res/css/admin/dishOrSetManage.css">
...
<body>
 ...
 <div class="fenye" style=" text-align: center">
  ...
  <a href="javascript:void(0)" onclick="contentSet('dish/TypeForDish.do?pageNow=1')">首页</a>
  <c:choose>
    <c:when test="${page.pageNow - 1 > 0}">
      <a href="javascript:void(0);" onclick="contentSet('dish/TypeForDish.do?pageNow=${page.pageNow - 1}')" >上一页</a>
    </c:when>
    <c:when test="${page.pageNow - 1 <= 0}">
      <a href="javascript:void(0);" onclick="contentSet('dish/TypeForDish.do?pageNow=1')">上一页</a>
    </c:when>
  </c:choose>
  <font size="2">第 ${page.pageNow}/ ${page.totalPageCount} 页</font>
  <c:choose>
    <c:when test="${page.pageNow + 1 < page.totalPageCount}">
      <a href="javascript:void(0);" onclick="contentSet('dish/TypeForDish.do?pageNow=${page.pageNow + 1}')">下一页</a>
    </c:when>
    <c:when test="${page.pageNow + 1 >= page.totalPageCount}">
      <a href="javascript:void(0);" onclick="contentSet('dish/TypeForDish.do?pageNow=${page.totalPageCount}')">下一页</a>
    </c:when>
  </c:choose>
  <a href="javascript:void(0);" onclick="contentSet('dish/TypeForDish.do?pageNow=${page.totalPageCount}')">尾页</a>
  <br>
  </div>
  </div>
</body>
