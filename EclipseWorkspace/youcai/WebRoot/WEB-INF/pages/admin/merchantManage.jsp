...
<body>
  ...
  <div class="fenye">
  <a class="FirsAndLast" href="javascript:void(0)" onclick="contentSet('manageMer/show.do?pageNow=1')">首页</a>
  <c:choose>
    <c:when test="${page.pageNow - 1 > 0}">
      <a href="javascript:void(0);" onclick="contentSet('manageMer/show.do?pageNow=${page.pageNow - 1}')" >上一页</a>
    </c:when>
    <c:when test="${page.pageNow - 1 <= 0}">
      <a href="javascript:void(0);" onclick="contentSet('manageMer/show.do?pageNow=1')">上一页</a>
    </c:when>
  </c:choose>
    <font size="2">第 ${page.pageNow}/${page.totalPageCount}页</font>
  <c:choose>
    <c:when test="${page.pageNow + 1 < page.totalPageCount}">
      <a href="javascript:void(0);" onclick="contentSet('manageMer/show.do?pageNow=${page.pageNow + 1}')">下一页</a>
    </c:when>
    <c:when test="${page.pageNow + 1 >= page.totalPageCount}">
      <a href="javascript:void(0);" onclick="contentSet('manageMer/show.do?pageNow=${page.totalPageCount}')">下一页</a>
    </c:when>
  </c:choose>
  <a class="FirsAndLast" href="javascript:void(0);" onclick="contentSet('manageMer/show.do?pageNow=${page.totalPageCount}')">尾页</a>
  </div>
